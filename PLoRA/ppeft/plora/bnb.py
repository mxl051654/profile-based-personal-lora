...
# from peft.tuners.lora.layer import PLoraLayer
from .layer import PLoraLayer

if is_bnb_available():

    class Linear8bitLt(torch.nn.Module, PLoraLayer):
...
if is_bnb_4bit_available():
    ...
    class Linear4bit(torch.nn.Module, PLoraLayer):
        # Lora implemented in a dense layer
        def __init__(
                self,
                base_layer: torch.nn.Module,
        ...
        def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
            ...
            if self.disable_adapters:
                ...
                result = self._mixed_batch_forward(x, *args, adapter_names=adapter_names, **temp_kwargs)
            elif self.merged:
                result = self.base_layer(x, *args, **temp_kwargs)
            else:
                result = self.base_layer(x, *args, **temp_kwargs)
                ...
                for active_adapter in self.active_adapters:
                    ...
                    #     output = lora_B(lora_A(dropout(x)) + lora_P(dropout(ep))) * scaling
                    #     # TODO  concat
                    # else:
                    #     x = dropout(x)
                    #     output = self.lora_magnitude_vector[active_adapter](
                    ...
                    if self.expert_num == 1:
                        ...
                        if not self.use_dora[active_adapter]:
                            ...
                            # h_a = lora_A(dropout(x))  # (bs, seq, dim)
                            # h_p = lora_P(dropout(ep))
                            # result = result + lora_B(torch.cat([h_a, h_p], dim=-1)) * scaling
                            ...
                            result = result.to(lora_A[0].weight.dtype)
    ...
    def dispatch_bnb_4bit(target: torch.nn.Module, adapter_name: str, **kwargs):
        ...
        if isinstance(target, BaseTunerLayer):
            ...
            target_base_layer = target  # Linear4bit(in_features=2048, out_features=2048, bias=False)
