...
from ..config import PeftConfig  # TODO 使用本地定义

# from peft.utils import PeftType
from ..peft_types import PeftType  # TODO 自定义 peft type 类型
...
class PLoraConfig(PeftConfig):
    ...
    """
    user_size: int = field(default=10)
    user_feature_dim: int = field(default=768)

    personal_type: str = field(default='profile')  # uid, profile
    inject_method: str = field(default='add')  #  add cat

    expert_num: int = field(default=1)  # 1 for lora/plora  >1 for moe
    # TODO 目标层
    target_layers: Optional[Union[list[str], str]] = field(default='all')
    # TODO 模块掩码（True for 2 mask module）
    module_mask: Optional[dict] = field(default=None)
