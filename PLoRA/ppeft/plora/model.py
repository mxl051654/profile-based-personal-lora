...
from .layer import Conv2d, dispatch_default, PLoraLayer
# from bitsandbytes.nn.modules import Linear4bit
from .bnb import Linear4bit  # Linear4bit(torch.nn.Module, PLoraLayer)
...
class PLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name) -> None:
        ...
        # TODO  add user embedding
        if self.peft_config[adapter_name].personal_type == 'uid':
            if self.peft_config[adapter_name].user_size is not None \
                    and self.peft_config[adapter_name].user_feature_dim is not None:
                # 使用 Embedding 学习用户嵌入
                # TODO  需要处理并行情况， 即adapter在多个设备
                self.lora_embedding = torch.nn.Embedding(
                    self.peft_config[adapter_name].user_size,
                    self.peft_config[adapter_name].user_feature_dim,
                    # TODO 默认正态随机初始化
                    _weight=torch.zeros(
                        self.peft_config[adapter_name].user_size,
                        self.peft_config[adapter_name].user_feature_dim)
        ...
        elif self.peft_config[adapter_name].personal_type == 'profile':
            pass
        else:
            print(f'No implementation for personal input {self.peft_config.personal_type}')
    ...
    def _prepare_model(self, peft_config: PLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        # call at BaseTuner.inject_adapter
        # TODO 修改目标模块选择逻辑 => for adapter_prune
        # TODO 遍历目标模块(target)，实现adapter插入
        ...
        kwargs = {
            ...
            #  TODO custom kwargs  => for adapter
            "user_size": self.peft_config[adapter_name].user_size,
            "user_feature_dim": self.peft_config[adapter_name].user_feature_dim,
            "personal_type": self.peft_config[adapter_name].personal_type,
            "expert_num": self.peft_config[adapter_name].expert_num,
            'inject_method': self.peft_config[adapter_name].inject_method,
        }
        ...
        if isinstance(target, PLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
            ...
            # TODO 创建adapter  => dispatcher()
            new_module = self._create_new_module(lora_config, adapter_name, target, **kwargs)
            if adapter_name not in self.active_adapters:
                # adding an additional adapter: it is not automatically trainable
                # TODO 修改 为 adapter 默认为 trainable
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        # TODO 后续 考虑 如何使用 _custom_modules
        ...
        if is_bnb_4bit_available():
            ...
            dispatchers.append(dispatch_bnb_4bit)  # for Linear4bit
        ...
        dispatchers.extend(
            [
                ...
                dispatch_hqq,
                dispatch_megatron,
                dispatch_default,  # Linear => PLinear(PLoraLayer)
            ]
        )

        new_module = None
        for dispatcher in dispatchers:
            # TODO  初始化 Adapter (注意优先级)
            # <class 'ppeft.plora.bnb.Linear4bit'>
