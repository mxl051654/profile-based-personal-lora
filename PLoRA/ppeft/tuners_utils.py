...
class BaseTuner(nn.Module, ABC):
    ...
    def inject_adapter(
        ...
        for key in key_list:
            ...
            # TODO check target_layer
            if not check_target_layer_exists(peft_config, key):
                continue

            # TODO check mask
            if isinstance(peft_config.module_mask, dict):
                if check_module_mask(peft_config, key):
                    continue
...
def check_target_layer_exists(config, key: str) -> bool | re.Match[str] | None:
    """A helper method to check if the passed module's key name matches any of the target modules in the adapter_config.
    ...
    if 'all' in config.target_layers:
        return True
    else:
        # layer_index = re.match(r".*\.[^.]*\.(\d+)\.", key).group(1)
        temp = re.compile(r'.block\.(\d+)')
        block_index = temp.search(key).group(1) if temp.search(key) is not None else ''
        l_map = {'e': 'encoder', 'd': 'decoder'}
        for l in config.target_layers:  # [e1, d2, e3]
            lt, li = l[0], l[1:]

            if l_map[lt] in key and li == block_index:
                return True
        return False


def check_module_mask(peft_config, key: str) -> bool | re.Match[str] | None:
    module_mask = peft_config.module_mask
    mk = {'.'.join(k.split('.')[2:-2]): v for k, v in module_mask.items()}
    if key in mk.keys():
        # print(f'mask {key} {mk[key]}')
        return mk[key]
    else:
        # print(f'not in {key}')
        return False
