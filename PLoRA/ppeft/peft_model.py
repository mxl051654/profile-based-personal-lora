...
from .pplug import (
    PPromptEncoderConfig, PPromptTuningConfig, PPrefixTuningConfig,
    PPrefixEncoder,
)
...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.PPREFIX_TUNING: PPrefixEncoder,
}
    ...
    **Attributes**:
    ...
    def __init__(
        ...
        if self._is_prompt_learning:
            ...
            # TODO  cls <class 'LaMP.PLoRA.ppeft.plora.model.PLoraModel'>
            self._peft_config = None
            cls = PEFT_TYPE_TO_MODEL_MAPPING[peft_config.peft_type]
            # TODO  {adapter_name: peft_config}  设计成字典 用以支持多 adapter
    ...
    def _setup_prompt_encoder(self, adapter_name: str):
        ...
        # TODO
        elif config.peft_type == PeftType.PPREFIX_TUNING:
            prompt_encoder = PPrefixEncoder(config)
        else:
    ...
    def get_prompt_embedding_to_save(self, adapter_name: str) -> torch.Tensor:
        ...
        # PREFIX_TUNING or P_TUNINGv2
        if self.peft_config[adapter_name].peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
    ...
    def get_prompt(self, batch_size: int,
        ...
        #  prefix/p-tuning v2  基于 pask_key_values
        if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:

            # TODO 判断 p / personal_type 类别  uid, profile , history
            if peft_config.personal_type == 'uid':
                user_size = peft_config.user_size  # 783
                num_input_token = peft_config.num_virtual_tokens - user_size
                p_token_num = 1
                # prompt_tokens = prompt_tokens[:, : peft_config.num_virtual_tokens]
                prompt_tokens = prompt_tokens[:, : num_input_token - p_token_num]
                personal_tokens = (kwargs.get('p').to(self.device) + num_input_token).unsqueeze(1)  # (batch_size, 1)
                # TODO: 拼接个性化tokens
                prompt_tokens = torch.cat([personal_tokens, prompt_tokens], dim=1)  # (batch_size, n)

                if peft_config.inference_mode:
                    # 推理阶段直接使用embedding
                    past_key_values = prompt_encoder.embedding.weight.repeat(batch_size, 1, 1)
                else:
                    # TODO： 修改encoder(PPrefixEncoder.forward())传入参数
                    past_key_values = prompt_encoder(prompt_tokens)

            elif peft_config.personal_type in ['profile', 'history']:
                # 划分为 task & personal input
                prompt_tokens = prompt_tokens[:, :  peft_config.num_virtual_tokens - peft_config.user_size]

                past_key_values = prompt_encoder(prompt_tokens, kwargs.get('p').to(self.device))
            ...
            past_key_values = past_key_values.view(
                batch_size,
                peft_config.num_virtual_tokens,  # input token num
                peft_config.num_layers * 2,  # layer * (key, value)
                peft_config.num_attention_heads,  # 注意力头数量
                peft_config.token_dim // peft_config.num_attention_heads,  # 嵌入由多注意力输出拼接
            )
            if peft_config.num_transformer_submodules == 2:
                past_key_values = torch.cat([past_key_values, past_key_values], dim=2)

            # 调整顺序 => (kv_pair_num. batch_size, attention_head_hum, virtual_token_num, head_dimension)
            past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(
                peft_config.num_transformer_submodules * 2
            )
            #  后处理模块 bloom_model_postprocess_past_key_value
            if TRANSFORMERS_MODELS_TO_PREFIX_TUNING_POSTPROCESS_MAPPING.get(self.config.model_type, None) is not None:
                post_process_fn = TRANSFORMERS_MODELS_TO_PREFIX_TUNING_POSTPROCESS_MAPPING[self.config.model_type]
                past_key_values = post_process_fn(past_key_values)
            return past_key_values
        else:
            # prompt_tuning/p-tuning 只需要 添加 input_embedding
    ...
    **Attributes**:
    ...
    def forward(
        ...
        if not peft_config.is_prompt_learning:
            # TODO add p embedding
            if p is not None:
                try:
                    # 如果有 lora_embedding
                    p = self.base_model.lora_embedding(p)
                except Exception as e:
                    p = p.to(self.base_model.device)
        ...
        if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
            ...
            # TODO
            prompts = self.get_prompt(batch_size=batch_size, task_ids=task_ids, **kwargs)
    ...
    def _prefix_tuning_forward(
        ...
        batch_size = _get_batch_size(input_ids, inputs_embeds)
        # TODO
        past_key_values = self.get_prompt(batch_size, **kwargs)
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        if not peft_config.is_prompt_learning:
            ...
            with self._enable_peft_forward_hooks(**kwargs):
                kwargs = {k: v for k, v in kwargs.items() if k not in self.special_peft_forward_args}
                # TODO enter <class 'LaMP.PLoRA.ppeft.plora.model.PLoraModel'>
                return self.base_model(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    inputs_embeds=inputs_embeds,
        ...
        if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
            past_key_values = self.get_prompt(batch_size, p=p)
            return self.base_model(
            ...
            prompts = self.get_prompt(batch_size=batch_size, task_ids=task_ids, p=p)
            prompts = prompts.to(inputs_embeds.dtype)
            inputs_embeds = torch.cat((prompts, inputs_embeds), dim=1)
            return self.base_model(inputs_embeds=inputs_embeds,
                                   # TODO
    ...
    def generate(self, *args, **kwargs):
        ...
        try:
            if not peft_config.is_prompt_learning:  # TODO lora enter
                # TODO  嵌入P
                if kwargs.get('p') is not None:
                    try:
                        kwargs['p'] = self.base_model.lora_embedding(kwargs.get('p'))  # (bs, 768)
                    except Exception as e:
                        pass
                with self._enable_peft_forward_hooks(*args, **kwargs):
                    kwargs = {k: v for k, v in kwargs.items() if k not in self.special_peft_forward_args}
    ...
    def prepare_inputs_for_generation(self, *args, task_ids: Optional[torch.Tensor] = None, **kwargs):
        ...
        if peft_config.is_prompt_learning:
            ...
            if model_kwargs["past_key_values"] is None \
                    and peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
                past_key_values = self.get_prompt(batch_size=model_kwargs["input_ids"].shape[0], **kwargs)
                ...
                if model_kwargs["past_key_values"] is None:
                    ...
                    prompts = prompts.to(inputs_embeds.dtype)
                    model_kwargs["inputs_embeds"] = torch.cat((prompts, inputs_embeds), dim=1, **kwargs)
...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def forward(
        ...
        peft_config = self.active_peft_config
        if not peft_config.is_prompt_learning:  # for lora like adapter
            # TODO add p embedding
            if p is not None:
                try:  # 如果有 lora_embedding
                    p = self.base_model.lora_embedding(p)
                except Exception as e:
                    p = p.to(self.base_model.device)
        ...
        if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
            ...
            prompts = self.get_prompt(batch_size=batch_size, p=p)
            prompts = prompts.to(inputs_embeds.dtype)
        ...
        try:
            if not peft_config.is_prompt_learning:
                ...
                if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
                    ...
                    prompts = self.get_prompt(batch_size=batch_size, task_ids=kwargs.pop("task_ids", None), **kwargs)
                    prompts = prompts.to(inputs_embeds.dtype)
    ...
    **Attributes**:
    ...
    def forward(
        ...
        if peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PPREFIX_TUNING]:
            ...
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
            prompts = self.get_prompt(batch_size=batch_size, **kwargs)
            prompts = prompts.to(inputs_embeds.dtype)
            inputs_embeds = torch.cat((prompts, inputs_embeds), dim=1)
            return self.base_model(inputs_embeds=inputs_embeds, **kwargs)
