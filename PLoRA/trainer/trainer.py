...
class Trainer:
    ...
    def set_signature_columns_if_needed(self):
        if self.signature_columns is None:
            # Inspect model forward signature to keep only the arguments it accepts.
            model_to_inspect = self.model
            if _is_peft_model(self.model):
                if hasattr(self.model, "get_base_model"):  # True
                    # PeftModel
                    model_to_inspect = self.model.get_base_model()
                else:
                    # PeftMixedModel do not provide a `get_base_model` method
                    model_to_inspect = self.model.base_model.model
            # TODO 检查基座模型的输入，确认包括 自定义参数
            signature = inspect.signature(model_to_inspect.forward)
            self.signature_columns = list(signature.parameters.keys())
            # Labels may be named label or label_ids, the default data collator handles that.
            self.signature_columns += list(set(["label", "label_ids"] + self.label_names))

    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        if not self.args.remove_unused_columns:
            return dataset
        self.set_signature_columns_if_needed()
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (total_batched_samples % args.gradient_accumulation_steps == 0
                    or is_last_step_and_steps_less_than_grad_acc   # last step
                ):
                    ...
                    if args.max_grad_norm is not None and args.max_grad_norm > 0:
                        ...
                        if is_sagemaker_mp_enabled() and args.fp16:
                            ...
                            # TODO  Apex (A PyTorch Extension) for AMP(自动混合精度)
                            ...
                            # TODO 如果不是分布式，实际调用torch实现
                            # TODO torch.nn.utils.clip_grad_norm_(parameters, max_norm, norm_type=norm_type)
                    ...
                    # TODO check grad  [opt.zero_grad()清空梯度, loss.backward()梯度下降, opt.step()参数更新 ]
                    self.optimizer.step()

                    # lora_emb
                    # {n: p.grad.norm() for n, p in model.named_parameters() if 'lora_emb' in n}
                    # lora 梯度
                    # {n: p.grad.norm() for n, p in model.named_parameters() if 'lora' in n}
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        if len(inputs) == 0:
            raise ValueError(
                ...
                f"training dataset contains keys expected by the model: {','.join(self.signature_columns)}."
            )
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.use_apex:
            ...
            # TODO 缩放(为了梯度累积) & 调用 loss.backward(**kwargs)
    ...
    def evaluation_loop(
        ...
        # TODO  容器，记录中间结果 intermediate results of evaluation loop
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # TODO: create accelerator object  for distributed training (on multi-GPU, TPU) or mixed precision training.
