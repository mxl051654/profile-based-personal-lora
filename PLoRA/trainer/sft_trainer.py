...
from trainer.trainer import Trainer  # TODO  处理 peft
# from trainer import Trainer
if is_peft_available():
    # TODO 修改 get_peft_model
    # from peft import PeftConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    from peft import prepare_model_for_kbit_training
    from ppeft import PeftConfig, PeftModel, get_peft_model
...
class SFTTrainer(Trainer):
        ...
        if is_peft_available() and peft_config is not None:
            ...
            if not isinstance(model, PeftModel):
                ...
                if (
                    ...
                    # TODO init peft_model
                    model = get_peft_model(model, peft_config)
                    # manual set reqires_grad
    ...
    def _prepare_dataset(
        ...
        if not packing:
            ...
            """
            packing通常用于将多个输入序列合并（或“打包”）成一个批次，以更高效地利用GPU内存和计算资源
            Packing将多个文本序列连接成一个长序列。每个文本序列的起始和结尾用特殊标记（如[CLS]和[SEP]）隔开
            """
    ...
    def _prepare_non_packed_dataloader(
        ...
        signature_columns = ["input_ids", "labels", "attention_mask", "p"]
        ...
        # TODO  处理需要删除的columns  不可行，在处理数据时，还没有初始化Trainer
        ...
        map_kwargs = {
            "batched": True,
            "remove_columns": extra_columns if remove_unused_columns else None,
