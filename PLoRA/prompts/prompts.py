...
from prompts.utils import extract_strings_between_quotes, extract_after_article, extract_after_review, \
    extract_after_paper, add_string_after_title, extract_after_colon, extract_after_description, extract_after_abstract
from prompts.contriever_retriever import retrieve_top_k_with_contriever
import random


def classification_citation_query_corpus_maker(inp, profile):  # LaMP_1
    corpus = [f'{x["title"]} {x["abstract"]}' for x in profile]
    # TODO
    inp = inp.split('For an author')[-1]
    extracted = extract_strings_between_quotes(inp)
    query = f'{extracted[1]} {extracted[2]}'
    return corpus, query


def classification_news_query_corpus_maker(inp, profile):  # lamp2 old
    corpus = [f'{x["title"]} {x["text"]}' for x in profile]
    query = extract_after_article(inp)
    return corpus, query


def classification_movies_query_corpus_maker(inp, profile):  # lamp 2 new
...
def classification_review_query_corpus_maker(inp, profile):  # lamp 3
    corpus = [f'{x["text"]}' for x in profile]
    query = extract_after_review(inp)
    return corpus, query


def generation_news_query_corpus_maker(inp, profile):  # lamp 4
    corpus = [f'{x["title"]} {x["text"]}' for x in profile]
    query = extract_after_article(inp)
    return corpus, query


def generation_paper_query_corpus_maker(inp, profile):  # lamp 5
    corpus = [f'{x["title"]} {x["abstract"]}' for x in profile]
    query = extract_after_paper(inp)
    return corpus, query


def generation_paper_long_query_corpus_maker(inp, profile):  # LaMP_long-5
    corpus = [f'{x["title"]} {x["abstract"]}' for x in profile]
    query = extract_after_abstract(inp)
    return corpus, query


def generation_avocado_query_corpus_maker(inp, profile):  # lamp 6
    corpus = [f'{x["text"]}' for x in profile]
    query = extract_after_colon(inp)
    return corpus, query


def generation_avocado_long_query_corpus_maker(inp, profile):  # LaMP_long-6
    corpus = [f'{x["text"]} {x["title"]}' for x in profile]
    query = extract_after_colon(inp)
    return corpus, query


def parphrase_tweet_query_corpus_maker(inp, profile):  # lamp 7
    corpus = [f'{x["text"]}' for x in profile]
    query = extract_after_colon(inp)
    return corpus, query
...
def create_classification_news_prompt(inp, profile, max_length, tokenizer):  # good
    ...
    for i, p in enumerate(profile):
        needed_part_len = len(
            tokenizer(f'**Demonstration{i}**\nthe category for the article: " " is "{p["category"]}" ')['input_ids'])
        ...
        prompt = f'**Demonstration{i}**\nthe category for the article: "{new_text}" is "{p["category"]}" '
        prompts.append(prompt)
...
def create_classification_movies_prompt(inp, profile, max_length, tokenizer):  # good
    ...
    for p in profile:
        ...
        tokens = tokenizer(p["description"], max_length=per_p_max_length + saved_tokens - needed_part_len,
                           truncation=True)
        saved_tokens += per_p_max_length - len(tokens['input_ids']) - needed_part_len
        new_text = tokenizer.batch_decode([tokens['input_ids']], skip_special_tokens=True)[0]
...
def create_classification_review_prompt(inp, profile, max_length, tokenizer):
    ...
    for i, p in enumerate(profile):
        ...
        needed_part_len = len(tokenizer(f'**Demonstration{i}**\n{p["score"]} is the score for " " ')['input_ids'])
        tokens = tokenizer(p["text"], max_length=per_p_max_length + saved_tokens - needed_part_len, truncation=True)
        saved_tokens += per_p_max_length - len(tokens['input_ids']) - needed_part_len
        new_text = tokenizer.batch_decode([tokens['input_ids']], skip_special_tokens=True)[0]
        # prompt = f'{p["score"]} is the score for "{new_text}" '
        prompt = f'**Demonstration{i}**\n{p["score"]} is the score for "{new_text}" '
        prompts.append(prompt)
    return f'{", and ".join(prompts)}. {inp}'
...
def create_generation_news_prompt(inp, profile, max_length, tokenizer):
    ...
    prompts = []
    for i, p in enumerate(profile):
        needed_part_len = len(tokenizer(f'**Demonstration{i}**\n"{p["title"]}" is the title for " " ')['input_ids'])
        tokens = tokenizer(p["text"], max_length=per_p_max_length + saved_tokens - needed_part_len, truncation=True)
        saved_tokens += per_p_max_length - len(tokens['input_ids']) - needed_part_len
        new_text = tokenizer.batch_decode([tokens['input_ids']], skip_special_tokens=True)[0]
        prompt = f'**Demonstration{i}**\n"{p["title"]}" is the title for "{new_text}" '
        prompts.append(prompt)
    return f'{", and ".join(prompts)}. {inp}'


def create_generation_paper_prompt(inp, profile, max_length, tokenizer):
    per_p_max_length = (max_length - 1 - 2 * (len(profile) - 1) - len(
        tokenizer("Following the given patterns")['input_ids'])) // len(profile)
    saved_tokens = 0
    prompts = []
    for i, p in enumerate(profile):
        needed_part_len = len(tokenizer(f'**Demonstration{i}**\n"{p["title"]}" is a title " " ')['input_ids'])
        ...
        prompt = f'**Demonstration{i}**\n"{p["title"]}" is a title for "{new_asbtract}" '
        prompts.append(prompt)
...
def create_generation_paper_long_prompt(inp, profile, max_length, tokenizer):
    ...
    prompts = []
    for i, p in enumerate(profile):
        needed_part_len = len(tokenizer(f'**Demonstration{i}**\n"{p["title"]}" is the title " " ')['input_ids'])
        tokens = tokenizer(p["abstract"], max_length=per_p_max_length + saved_tokens - needed_part_len, truncation=True)
        saved_tokens += per_p_max_length - len(tokens['input_ids']) - needed_part_len
        new_asbtract = tokenizer.batch_decode([tokens['input_ids']], skip_special_tokens=True)[0]
        prompt = f'**Demonstration{i}**\n"{p["title"]}" is the title for "{new_asbtract}" '
        prompts.append(prompt)
    return f'{", and ".join(prompts)}. Following the given patterns {inp}'
...
def create_parphrase_tweet_prompt(inp, profile, max_length, tokenizer):
    per_p_max_length = (max_length - 1 - 2 * (len(profile) - 1) - len(
        tokenizer("are written by user. Following the given patterns")['input_ids'])) // len(profile)
    saved_tokens = 0
    prompts = []
    for p in profile:
...
def create_prompt_generator(num_retrieve, ret_type="bm25", is_ranked=False, max_length=512, tokenizer=None):
    ...
    if ret_type == "contriever" and not is_ranked:
        ranker_path = '/data/hf/facebook/contriever'
        tokenizer = AutoTokenizer.from_pretrained(ranker_path)
        contriever = AutoModel.from_pretrained(ranker_path).to("cuda")
        contriever.eval()
    ...
    def prompt(inp, profile, task):
        ...
        #     corpus, query = classification_review_query_corpus_maker(inp, profile)
        # else:
        #     print(f'no implementation for {task}')

        # if not is_ranked:  # 如果没有预先排序
        #     if ret_type == "bm25":
        ...
        #         selected_profs = retrieve_top_k_with_contriever(contriever, tokenizer, corpus, profile, query,
        #                                                         num_retrieve)
        ...
        if ret_type == "recency_contriever":
            ...
            selected_profs_cont = profile[:num_retrieve]  # 提取top-k profile
            selected_profs = selected_profs_cont

        factor = 0.6
        while True:
            try:
                # 约束history profile最大长度 , 确保 输入 可以完全保留
                ...
                # TODO
                    ...
                    print(f'no implementation for {task} @create_prompt_generator')
            except:
                factor -= 0.1
                if factor < 0:
                    print("not possible")
