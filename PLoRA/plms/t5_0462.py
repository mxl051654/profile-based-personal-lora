...
class T5Attention(nn.Module):
    def __init__(
        ...
        # TODO 后续 lora 会替换掉Linear模块，并支持自定义输入
    ...
    def forward(
        ...
        if isinstance(self.q, nn.Linear):
            ...
            query_states = self.q(hidden_states, **kwargs)
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            # TODO
            if isinstance(self.k, nn.Linear):
                key_states = self.k(current_states)
                value_states = self.v(current_states)
            else:
                key_states = self.k(current_states, **kwargs)
                value_states = self.v(current_states, **kwargs)
