...
class T5DenseActDense(nn.Module):
    ...
    def forward(self, hidden_states, **kwargs):
        if isinstance(self.wi, nn.Linear):
            hidden_states = self.wi(hidden_states, )  # TODO
        else:
            hidden_states = self.wi(hidden_states, **kwargs)
        ...
        if isinstance(self.wo, nn.Linear):
            hidden_states = self.wo(hidden_states)  # TODO
        else:
            hidden_states = self.wo(hidden_states, **kwargs)
...
class T5DenseGatedActDense(nn.Module):
    ...
    def forward(self, hidden_states, **kwargs):
        if isinstance(self.wi_0, nn.Linear):
            hidden_gelu = self.act(self.wi_0(hidden_states))  # TODO
        else:
            hidden_gelu = self.act(self.wi_0(hidden_states, **kwargs))  # TODO

        if isinstance(self.wi_1, nn.Linear):
            hidden_linear = self.wi_1(hidden_states)  # TODO
        else:
            hidden_linear = self.wi_1(hidden_states, **kwargs)  # TODO
        ...
        if isinstance(self.wo, nn.Linear):
            ...
            hidden_states = self.wo(hidden_states, **kwargs)  # TODO
        return hidden_states
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states,
        ...
        forwarded_states = self.DenseReluDense(forwarded_states, **kwargs)
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if key_value_states is None:
                ...
                if isinstance(proj_layer, nn.Linear):
                    ...
                    hidden_states = shape(proj_layer(hidden_states, **kwargs))
                ...
                if isinstance(proj_layer, nn.Linear):
                    ...
                    hidden_states = shape(proj_layer(key_value_states, **kwargs))
        ...
        # get query states
        # TODO  <class 'ppeft.plora.layer.PLinear'>
        if isinstance(self.q, nn.Linear):
            query_states = shape(self.q(hidden_states))
        else:
            query_states = shape(self.q(hidden_states, **kwargs))
        ...
        if isinstance(self.o, nn.Linear):
            attn_output = self.o(attn_output)
        else:
            attn_output = self.o(attn_output, **kwargs)
...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # TODO custom loss
            # orthogonality_loss = []
            # if len(act_list_de) > 0:
            #     for acts in act_list_en + act_list_de:
            #         o_lambda = 5
            #         y1, y2 = acts
            #         hd = y1.shape[-1]
            #         cs = torch.cosine_similarity(y1.reshape(-1, hd), y2.reshape(-1, hd))
            #         orthogonality_loss.append(o_lambda * torch.abs(cs).mean())  # 平均绝对点积值
            # loss += torch.mean(torch.stack(orthogonality_loss))
