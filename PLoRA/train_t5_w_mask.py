...
def main():
    ...
    if model_args.peft_type == 'LORA':
        ...
        peft_config = PrefixTuningConfig(
            ...
            task_type="SEQ_2_SEQ_LM",
            # token_dim=model.config.token_dim,  # 768
            # num_transformer_submodules=model.config.num_transformer_submodules,
        ...
        # TODO  添加 mask from
        with open(f"/data/mxl/rlhf/LaMP/output/{task}/SFT_PLORA/module_importance.json", 'r') as f:
            module_k_v = json.load(f)

        def set_flags_by_percentage(input_dict, percentage=30):
            # 升序
            sorted_items = sorted(input_dict.items(), key=lambda item: item[1])
            # 计算设为 True 的数量
            n_true = int(len(sorted_items) * (percentage / 100))
            # 创建新字典
            result_dict = {}
            for idx, (k, v) in enumerate(sorted_items):
                if idx < n_true:
                    result_dict[k] = True  # 掩码低分选手
                else:
                    result_dict[k] = False

            return result_dict

        module_mask = set_flags_by_percentage(module_k_v, model_args.module_mask_percent)
        # TODO  modify tuner_utils.py  inject_adapter
        ...
        peft_config = PPrefixTuningConfig(
            ...
            personal_type=model_args.personal_type,
            user_size=model_args.user_size,  # load_dataset初始化
        )
        from ppeft.mapping import get_peft_model
        model = get_peft_model(model, peft_config)
